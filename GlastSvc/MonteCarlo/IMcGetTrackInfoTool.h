...
class IMcGetTrackInfoTool : virtual public IAlgTool 
{
 public:
    ...
    virtual int                           getNMcParticles(const Event::TkrTrack*)=0;

    /// @brief Return the vector of McParticles associated to a given TkrPatCand track
    virtual const Event::McParticleRefVec getMcParticleVec(const Event::TkrTrack*)=0;

    /// @brief Return TkrPatCand <-> McParticle "best" match
    virtual const Event::McParticle*      getBestMcParticle(const Event::TkrTrack*)=0;

    /// @brief Return TkrPatCand <-> McParticle "best" match
    virtual const Event::TkrTrack*        getBestTkrTrack(const Event::McParticle*)=0;

    /// @brief Return the number of hits on a given TkrPatCand track associated to
    ///        a given McParticle
    virtual int                           getNumMcHits(const Event::TkrTrack*, const Event::McParticle*)=0;
};
#endif
